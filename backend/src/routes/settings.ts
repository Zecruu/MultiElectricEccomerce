...
// Store Info
router.get('/store', requireAuth(), requireRole('employee'), async (_req, res) => {
  res.json(storeInfo);
});
router.patch('/store', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const before = { ...storeInfo };
  Object.assign(storeInfo, req.body || {});
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.store.update', before, after: storeInfo });
  res.json(storeInfo);
});
...
router.patch('/templates/:key', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  ...
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.templates.update', targetId: key, before, after: { subject: t.subject, body: t.body } });
  res.json(t);
});
router.post('/templates/:key/test', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const key = req.params.key;
  if (!templates[key]) return res.status(404).json({ error: 'Not found' });
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.templates.send-test', targetId: key });
  res.json({ message: 'Test sent (mock)' });
});

// Webhooks
router.get('/webhooks', requireAuth(), requireRole('admin'), async (_req, res) => {
  res.json({ endpoints: webhooks, deliveries: webhookDeliveries.slice(-50) });
});
router.post('/webhooks', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  ...
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.webhooks.create', targetId: id, after: item });
  res.status(201).json(item);
});
router.patch('/webhooks/:id', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const { id } = req.params; const idx = webhooks.findIndex(w => w.id === id);
  if (idx < 0) return res.status(404).json({ error: 'Not found' });
  const before = { ...webhooks[idx] };
  webhooks[idx] = { ...webhooks[idx], ...req.body, secret: '••••••' };
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.webhooks.update', targetId: id, before, after: webhooks[idx] });
  res.json(webhooks[idx]);
});
router.delete('/webhooks/:id', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const { id } = req.params; const before = webhooks.find(w => w.id === id);
  if (!before) return res.status(404).json({ error: 'Not found' });
  webhooks = webhooks.filter(w => w.id !== id);
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.webhooks.delete', targetId: id, before });
  res.status(204).send();
});
router.post('/webhooks/:id/replay', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const { id } = req.params; if (!webhooks.find(w => w.id === id)) return res.status(404).json({ error: 'Not found' });
  ...
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.webhooks.replay', targetId: id, after: d });
...
router.patch('/security', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const before = { ...security };
  security = { ...security, ...(req.body || {}) };
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.security.update', before, after: security });
  res.json(security);
});
...
router.patch('/integrations', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  const before = { ...integrations };
  integrations = { ...integrations, ...(req.body || {}) };
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.integrations.update', before, after: integrations });
  res.json(integrations);
});
router.post('/integrations/healthcheck', requireAuth(), requireRole('admin'), async (req: AuthRequest, res) => {
  // Simulate a health check
  integrations.email.lastCheck = new Date().toISOString();
  await AuditLog.create({ actorId: req.user!.id as any, action: 'settings.integrations.healthcheck' });
