...
async function ensureAudit(req, section) {
    try {
        await AuditLog_1.AuditLog.create({
            actorId: req.user ? new mongoose_1.Types.ObjectId(req.user.id) : null,
...
router.get('/sales', (0, auth_1.requireAuth)(), (0, auth_1.requireRole)('admin'), async (req, res) => {
    await ensureAudit(req, 'sales');
    const { from, to } = parseRange(req.query);
    const { category = '', employee = '', page = '1', limit = '50' } = req.query;
    ...
    if (category) {
        const prods = await Product_1.Product.find({ category: String(category) }).select('_id').lean();
        ...
        filter['items.productId'] = { $in: Array.from(productIdSet).map(id => new mongoose_1.Types.ObjectId(id)) };
    }
    ...
    const allOrders = await Order_1.Order.find(filter).select('total email items createdAt').lean();
    const totalRevenue = allOrders.reduce((s, o) => s + (o.total || 0), 0);
    const totalOrders = allOrders.length;
    const avgOrderValue = totalOrders ? totalRevenue / totalOrders : 0;
    const activeCustomers = new Set(allOrders.map((o) => (o.email || '').toLowerCase())).size;
    ...
    for (const [sku, v] of qtyBySku.entries()) {
        if (!topProduct || v.qty > topProduct.qty)
            topProduct = { sku, name: v.name, qty: v.qty };
    }
...
router.get('/inventory', (0, auth_1.requireAuth)(), (0, auth_1.requireRole)('admin'), async (req, res) => {
    await ensureAudit(req, 'inventory');
    const { category = '' } = req.query;
    const filter = {};
    if (category)
        filter.category = String(category);
    const products = await Product_1.Product.find(filter).select('sku category price stock lowStockThreshold status translations').lean();
    const low = products.filter((p) => (p.stock || 0) <= (p.lowStockThreshold || 0));
...
router.get('/customers', (0, auth_1.requireAuth)(), (0, auth_1.requireRole)('admin'), async (req, res) => {
    await ensureAudit(req, 'customers');
    const { from, to } = parseRange(req.query);
    const filter = { createdAt: { $gte: from, $lte: to } };
    const orders = await Order_1.Order.find(filter).select('email total createdAt').lean();
    // Determine if a customer is "new" in this window
    const emails = Array.from(new Set(orders.map((o) => (o.email || '').toLowerCase())));
    let newCount = 0, returningCount = 0;
    for (const e of emails) {
        const first = await Order_1.Order.findOne({ email: e }).sort({ createdAt: 1 }).select('createdAt').lean();
        if (first && first.createdAt >= from && first.createdAt <= to)
            newCount++;
        else
            returningCount++;
    }
...
router.get('/employees', (0, auth_1.requireAuth)(), (0, auth_1.requireRole)('admin'), async (req, res) => {
